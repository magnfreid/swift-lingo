...
final class GameManager {
    ...
    private var currentDifficulty: String {
        UserDefaultsManager.shared.getDifficulty()
    }
    private var currentPlayer: String {
        UserDefaultsManager.shared.getPlayerName()
    }
...
extension GameManager {
    ...
    func checkForBadgesAfterGame(score: Int, totalTurns: Int) -> [Badges] {
        ...
        print(
            "Player badges before firstTime check: \(BadgeManager.shared.getBadges(for: player))"
        )
       ...
       //"🔑 Scores"
        ...
        if score == 0
            && !BadgeManager.shared.hasBadge(badges: .typoNoob, for: player)
        {
        ...
        if score == totalTurns - 1
            && !BadgeManager.shared.hasBadge(badges: .almostThere, for: player)
        {
        ...
        if hasDarkMode
            && !BadgeManager.shared.hasBadge(badges: .nightMode, for: player)
        {
        ...
        if name == String(name.reversed())
            && !BadgeManager.shared.hasBadge(badges: .palindrome, for: name)
        {
        ...
        if correctInARow >= 20 {
            ...
            case "easy":
                if !BadgeManager.shared.hasBadge(
                    badges: .easyStreak, for: player)
                {
                    BadgeManager.shared.addBadge(
                        badge: .easyStreak, for: player)
                    unlockedBadges.append(.easyStreak)
                }
            case "medium":
                if !BadgeManager.shared.hasBadge(
                    badges: .mediumStreak, for: player)
                {
                    BadgeManager.shared.addBadge(
                        badge: .mediumStreak, for: player)
                    unlockedBadges.append(.mediumStreak)
                }
            case "hard":
                if !BadgeManager.shared.hasBadge(
                    badges: .hardStreak, for: player)
                {
                    BadgeManager.shared.addBadge(
                        badge: .hardStreak, for: player)
                    unlockedBadges.append(.hardStreak)
                }
            case "extreme":
                if !BadgeManager.shared.hasBadge(
                    badges: .extremeStreak, for: player)
                {
                    BadgeManager.shared.addBadge(
                        badge: .extremeStreak, for: player)
        ...
        if currentDifficulty.lowercased() == "extreme" && score == totalTurns
            && !BadgeManager.shared.hasBadge(badges: .fullStreak, for: player)
        {
        ...
        if currentDifficulty.lowercased() == "hard" && score == totalTurns
            && !BadgeManager.shared.hasBadge(badges: .noMercy, for: player)
        {
        ...
        if allUnder4Seconds
            && !BadgeManager.shared.hasBadge(badges: .perfectTime, for: player)
        {
        ...
        if fastAnswers.count >= 10
            && !BadgeManager.shared.hasBadge(
                badges: .fastAndFurious, for: player)
        {
        ...
        if sheepTriggered
            && !BadgeManager.shared.hasBadge(badges: .sheep, for: player)
        {
